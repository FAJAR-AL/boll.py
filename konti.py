...
\_/   \_/ \|\____/\_/ \|\_/\_\
         \/    \/ 
 %s[%s*%s] By : %sRomi Afrizal&Riyandika 
 %s[%s*%s] --------------------------------------
 [%s*%s] IP : %s%s
...
def menu():
    ...
    #print (' [%s08%s] Info script dan recode'%(K,P))
    print (' [%s00%s] Keluar (hapus token)'%(M,P))
    unik = raw_input('\n%s [?] Menu : %s'%(P,K))
    if unik == '':
        print("%s [!] Isi yang benar cok "%(M));jeda(2);menu()
    elif unik in['1','01']:
        publik(romz)
    elif unik in['2','02']:
        followers(romz)
    	...
    	try:
            ...
            file = raw_input("%s [?] masukin file : "%(P));jeda(0.2)
            if file == "":
                print("%s [!] file gk ada "%(M))
...
def publik(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] Ketik '%sah%s' kalo mau ngews temen sendiri "%(P,M,P,H,P))
        idt = raw_input(' [*] Target ngews : %s'%(K))
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] menyusun rencana ngews :%s %s ' % (P,H,str(len(id))),
            sys.stdout.flush();jeda(0.0050)

        bff.close()
        print ('\n\n %s[%s√%s] Succes ngews temen %s%s'%(P,H,P,H,nm['name']))
        print ('%s [%s√%s] File ngews tersimpan :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
